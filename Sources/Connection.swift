...
open class Nats: NSObject, StreamDelegate {
    ...
    fileprivate var mode: NatsConnectMode
    ...
    public init(url: String,
                verbose: Bool = true,
                pedantic: Bool = false,
                mode: NatsConnectMode = .unAuthorization) {
    ...
    fileprivate func authorize(_ outStream: OutputStream, _ inStream: InputStream) {
        do {
            ...
            let newConfig = config.merging(self.mode.asPayload()) { (current, _) in current }
            
            let configData = try JSONSerialization.data(withJSONObject: newConfig, options: [])
